...
- #420：切换员工时岗位卡仍可显示上一位（#413 stale-while-revalidate），但裁撤 / 编辑只在 `card.data.id === selectedId` 时挂上。避免弹窗显示 A 的名字却对 B 发请求。
...
- #447：设置新增按项目保存、默认关闭的“智能协作建议 · 预览”。开启前展示 Jev 服务方与数据范围；上报中心只在主动点击时为最近最多 20 条失败升级记录生成参考建议。原有记录、顺序和追溯入口保留，关闭会取消待处理建议，服务不可用时仍可正常查看报告。
